...
import { setUserSelected } from "../features/permissionSlice";
...
export default function Datatable() {
  ...
  const goToUserPermissions = (userSelected) => {
    dispatch(setUserSelected(userSelected));
    navigate(`/mantencion-usuarios/${userSelected.ID}`);
  };
      ...
      render: (text, record) => {
        ...
        return user.users.length >= 1 ? (
          <Space>
            ...
            <Button
              type="success"
              onClick={() => goToUserPermissions(record)}
              disabled={editable}
            >
              Roles
            </Button>
